...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( esc_html__( 'Older posts', 'flatnatura' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts', 'flatnatura' ) ); ?></div>
			<?php endif; ?>
...
function flatnatura_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && flatnatura_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'flatnatura' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'flatnatura' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'flatnatura' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
